      ...
      <div class="leftBox">
        ...
        <div class="BtnBox">
          <el-button size="mini" icon="el-icon-thumb"
            >赞({{ videoInfo.praisedCount }})</el-button
          >
        ...
        <div class="jieBox">
          <div class="timeBox">
            发布时间:{{ videoInfo.publishTime | showDate
              ...
              >播放次数:{{
                videoInfo.playTime > 10000
                  ? (videoInfo.playTime / 10000).toFixed(0) + "万"
                  : videoInfo.playTime
              }}</span
            >
          </div>
...
export default {
  ...
  methods: {
    async getVideo(id) {
      const res = await this.$http.get("video/detail", { params: { id: id } });
    ...
    async getVideoComment(id) {
      this.videoComments = "";
      const res = await this.$http.get("/comment/video", {
        params: {
          id: id,
          limit: this.pageSize,
          offset: this.offset,
